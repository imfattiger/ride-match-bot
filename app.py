...
DISTRICT_DATA = {
    ...
    "新北市": ["板橋區", "中和區", "永和區", "三重區", "新莊區", "新店區", "林口區", "淡水區", "汐止區", "蘆洲區", "土城區", "樹林區", "五股區", "泰山區", "八里區", "鶯歌區", "三峽區", "瑞芳區", "金山區", "萬里區", "三芝區", "石門區", "坪林區", "烏來區"],
    "基隆市": ["仁愛區", "信義區", "中正區", "中山區", "安樂區", "暖暖區", "七堵區"],
    "桃園市": ["桃園區", "中壢區", "平鎮區", "八德區", "楊梅區", "蘆竹區", "龜山區", "龍潭區", "大溪區", "觀音區", "新屋區", "大園區", "復興區"],
    "新竹市": ["東區", "北區", "香山區"],
    "新竹縣": ["竹北市", "竹東鎮", "新埔鎮", "關西鎮", "湖口鄉", "新豐鄉", "芎林鄉", "寶山鄉", "橫山鄉", "北埔鄉", "峨眉鄉", "尖石鄉", "五峰鄉"],
    "苗栗縣": ["苗栗市", "頭份市", "竹南鎮", "後龍鎮", "通霄鎮", "苑裡鎮", "公館鄉", "大湖鄉", "卓蘭鎮", "西湖鄉", "頭屋鄉", "三義鄉", "造橋鄉", "三灣鄉", "南庄鄉", "獅潭鄉"],
    "台中市": ["西屯區", "北屯區", "南屯區", "東區", "南區", "西區", "北區", "大里區", "太平區", "豐原區", "沙鹿區", "清水區", "梧棲區", "大甲區", "大安區", "后里區", "石岡區", "東勢區", "新社區", "潭子區", "大雅區", "神岡區", "大肚區", "龍井區", "霧峰區", "烏日區", "外埔區", "和平區"],
    "彰化縣": ["彰化市", "員林市", "和美鎮", "鹿港鎮", "溪湖鎮", "二林鎮", "北斗鎮", "田中鎮", "福興鄉", "花壇鄉", "伸港鄉", "線西鄉", "芬園鄉", "大村鄉", "埔鹽鄉", "埔心鄉", "永靖鄉", "社頭鄉", "田尾鄉", "埤頭鄉", "溪州鄉", "竹塘鄉", "二水鄉", "芳苑鄉", "大城鄉"],
    "南投縣": ["南投市", "埔里鎮", "草屯鎮", "竹山鎮", "集集鎮", "名間鄉", "中寮鄉", "魚池鄉", "國姓鄉", "水里鄉", "信義鄉", "鹿谷鄉", "仁愛鄉"],
    "雲林縣": ["斗六市", "虎尾鎮", "西螺鎮", "土庫鎮", "北港鎮", "斗南鎮", "古坑鄉", "大埤鄉", "莿桐鄉", "林內鄉", "二崙鄉", "崙背鄉", "麥寮鄉", "東勢鄉", "褒忠鄉", "臺西鄉", "元長鄉", "四湖鄉", "口湖鄉", "水林鄉"],
    "嘉義市": ["東區", "西區"],
    "嘉義縣": ["太保市", "朴子市", "布袋鎮", "大林鎮", "民雄鄉", "水上鄉", "番路鄉", "梅山鄉", "竹崎鄉", "中埔鄉", "大埔鄉", "溪口鄉", "新港鄉", "六腳鄉", "東石鄉", "義竹鄉", "鹿草鄉", "阿里山鄉"],
    "台南市": ["永康區", "安南區", "東區", "北區", "南區", "安平區", "中西區", "仁德區", "歸仁區", "關廟區", "新市區", "善化區", "新化區", "安定區", "西港區", "七股區", "將軍區", "學甲區", "北門區", "新營區", "後壁區", "白河區", "東山區", "鹽水區", "柳營區", "下營區", "六甲區", "官田區", "麻豆區", "佳里區", "玉井區", "楠西區", "南化區", "龍崎區", "左鎮區", "大內區", "山上區"],
    "高雄市": ["三民區", "鳳山區", "左營區", "楠梓區", "前鎮區", "苓雅區", "鼓山區", "小港區", "新興區", "前金區", "鹽埕區", "旗津區", "仁武區", "大社區", "岡山區", "路竹區", "橋頭區", "燕巢區", "田寮區", "阿蓮區", "湖內區", "茄萣區", "永安區", "彌陀區", "梓官區", "大樹區", "鳥松區", "林園區", "大寮區", "旗山區", "美濃區", "六龜區", "甲仙區", "杉林區", "內門區"],
    "屏東縣": ["屏東市", "潮州鎮", "東港鎮", "恆春鎮", "萬丹鄉", "內埔鄉", "長治鄉", "麟洛鄉", "九如鄉", "里港鄉", "鹽埔鄉", "高樹鄉", "萬巒鄉", "竹田鄉", "新埤鄉", "枋寮鄉", "新園鄉", "崁頂鄉", "林邊鄉", "南州鄉", "佳冬鄉", "琉球鄉", "車城鄉", "滿州鄉", "枋山鄉"],
    "宜蘭縣": ["宜蘭市", "羅東鎮", "蘇澳鎮", "頭城鎮", "礁溪鄉", "冬山鄉", "五結鄉", "員山鄉", "壯圍鄉", "三星鄉", "大同鄉", "南澳鄉"],
    "花蓮縣": ["花蓮市", "鳳林鎮", "玉里鎮", "新城鄉", "吉安鄉", "壽豐鄉", "光復鄉", "瑞穗鄉", "富里鄉", "秀林鄉", "萬榮鄉", "卓溪鄉"],
    "台東縣": ["台東市", "成功鎮", "關山鎮", "卑南鄉", "鹿野鄉", "太麻里鄉", "東河鄉", "長濱鄉", "池上鄉", "大武鄉", "海端鄉", "延平鄉", "綠島鄉", "蘭嶼鄉"]
}
...
SCHEMA_MIGRATIONS = [
    ...
    (21, "ALTER TABLE matches ADD COLUMN IF NOT EXISTS vehicle_type TEXT",
...
<style>
  ...
  .uid{{max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px;font-family:monospace}}
...
def admin_community():
    ...
    try:
        ...
        for (uid,) in users:
            ...
            rows += (
                f"<tr>"
                f"<td class='uid' title='{uid}'>{uid or ''}</td>"
...
def handle_message(event):
    ...
    # --- 管理員指令 ---
        ...
        push_text = (
            "嗨！基隆社群的朋友 👋\n\n"
            "這則是給社群朋友的專屬廣播，感謝你們的加入！\n"
            "擔心大家不熟操作，簡單帶一下：\n\n"
            "發一筆行程按幾下就好，快的話 30 秒搞定：\n"
            "1️⃣ 點下方「我要載客/貨」或「我要搭車/寄物」\n"
            "2️⃣ 填好時間和路線\n"
            "3️⃣ 系統自動配對，配到了直接通知你\n\n"
            "基隆↔台北／南港／內科這條線現在有人在等配對，趕快試試看 🚗"
        )
        success, fail = 0, 0
